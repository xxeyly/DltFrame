...
{
    public class DownComponent : ComponentBase
    {
        public static DownComponent Instance;
        ...
        /// </summary>
        /// <returns></returns>
        public DownData GetCurrentDownComponentData()
        {
        ...
        /// <returns></returns>
        public DownData GetDownComponentDataByFileName(string fileName)
        {
            foreach (DownData downData in allDownComponentData)
            {
                if (downData.downName == DataComponent.AllCharToLower(fileName))
                {
                    return downData;
                }
            }
                    ...
                    Application.platform == RuntimePlatform.WindowsEditor)
                {
                    ...
                    if (File.Exists(downPath))
                    {
                        ...
                        if (fileSize != fileInfo.fileSize || MD5 != fileInfo.fileMd5)
                        {
                            ...
                            allDownComponentData.Add(downData);
                        }
                        else
                        {
        ...
        {
            //正在下载的与切入的是同一个，不需要切换
            if (_currentDownDataTask != null && _currentDownDataTask.downName == DataComponent.AllCharToLower(downName))
            {
        ...
        private void UpdateDownProgress(string downName, long downCurrentSize)
        {
            foreach (DownData downData in allDownComponentData)
            {
                if (downData.downName == downName)
                {
                    downData.downCurrentSize = downCurrentSize;
                    break;
                }
            }
        }
