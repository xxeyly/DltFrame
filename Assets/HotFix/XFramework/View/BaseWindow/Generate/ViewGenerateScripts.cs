...
#pragma warning restore 0414
        ...
        protected static string GetPath(string scriptName)
        {
            ...
            string[] path = AssetDatabase.FindAssets(scriptName);

            for (int i = 0; i < path.Length; i++)
            {
                Debug.Log(AssetDatabase.GUIDToAssetPath(path[i]));
                if (AssetDatabase.GUIDToAssetPath(path[i]).Contains("Assets") &&
                    Path.GetFileName(AssetDatabase.GUIDToAssetPath(path[i])) == (scriptName + ".cs"))
                {
